...
async def main():
    session = url_history.HistorySession("committee_cache.db")
    for start_year in range(2025, 2027, 2):
        ...
        all_meetings, meetings_by_bill = await load_biennium_meetings_by_bill(session, start_year, now, FORCE_FETCH)
        ...
        for bill_number in meetings_by_bill:
            ...
            for dt, meeting, item, meeting_dict in meetings:
                ...
                while not items:
                    response = await session.get(url, fetch_again=FORCE_FETCH, index=history_index)
                    if not response:
                        print(url)
                        break
                    history_index -= 1
                ...
                for item in items:
                    ...
                    testimonyOptions = await session.get(url)
