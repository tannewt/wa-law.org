...
for start_year in range(2023, 2027, 2):
    ...
    cur.execute("SELECT bills.rowid, prefix, number, COUNT(testifiers.sign_in_time) as ts FROM bills, testifiers, agenda_items, meetings WHERE datetime(meetings.start_time) < datetime(?) AND meetings.rowid = agenda_items.meeting_rowid AND bills.rowid = agenda_items.bill_rowid AND agenda_items.rowid = testifiers.agenda_item_rowid AND bills.biennium_rowid = ? GROUP BY bills.rowid ORDER BY ts DESC, number ASC", (now, biennium_rowid))
    for row in cur:
        counts = db.cursor()
        counts.execute("SELECT position, COUNT(testifiers.position_rowid) FROM positions, testifiers, agenda_items WHERE positions.rowid = testifiers.position_rowid AND agenda_items.rowid = testifiers.agenda_item_rowid AND bill_rowid = ? GROUP BY testifiers.position_rowid", (row[0],))
        position_counts = dict(counts)
    ...
    for bill_rowid, prefix, bill_number in cur:
        ...
        for action_date, history_line in bill_history:
            item = rfeed.Item(
                ...
                categories=["official"],
                guid=rfeed.Guid(action_date.split("T")[0]+"/"+history_line, isPermaLink=False),
